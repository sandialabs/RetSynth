    ...
    def add_metacyc_to_db(self):
        ...
        def compile_all_cpd_rxn_info(MC):
            ...
            for key, value in MC.all_rxns.iteritems():
                allreactions.append((key, value))
            for key, value in MC.all_cpds.iteritems():
                allcompounds.append((key, value[0], value[1]))
            ...
            for rxn in allreactions:
                ...
                if result is None:
                    ...
                    self.cnx.execute("INSERT INTO reaction VALUES (?,?,?)", (rxn[0], rxn[1], self.rxntype))
                    self.cnx.commit()
...
class MetaCyc(object):
    ...
    def compound_translator(self, compound_ID, biocyc_ID, inchi_ID, name, compartment):
        '''
        ...
        if biocyc_ID:
            ...
            try:
                ...
                for cpdID in cpdID_list:
                    ...
                    self.fill_compound_arrays(compound_ID, inchi_ID, cpdID, name, compartment)
            except KeyError:
                self.fill_compound_arrays(compound_ID, inchi_ID, compound_ID, name, compartment)
        else:
            self.fill_compound_arrays(compound_ID, inchi_ID, compound_ID, name, compartment)

    def fill_compound_arrays(self, ID, inchi, cpdID, name, compartment):
        '''
        Determines whether or not to add compound to compound arrays which will
        later be added to the sqlite database
        '''
        if not self.inchidb:
            self.arrays(cpdID+'_'+compartment, compartment, name)
            ...
            if not inchi:
                ...
                if cpdID.startswith('InChI'):
                    ...
                    if not inchi2:
                        ...
                        self.arrays(inchi2+'_'+compartment, compartment, name)
    ...
    def read_metacyc_file(self, BIOCYC_translator, file_name):
        ...
        for c in allspecies:
            count += 1
            print ('{} out of {} compounds'.format(count, len(allspecies)))
            attr = c.findAll('p')
            inchikey = None
            biocycID = None
            for a in attr:
            ...
            self.compound_translator(c.get('id'), biocycID, inchikey,
        ...
        for rxn in allreactions:
            ...
            if biocyc_rxn_ID in self.BIOCYC_translator['rxn']:
                for krxn in self.BIOCYC_translator['rxn'][biocyc[1]]:
                    self.retrieve_rxn_info(rxn, krxn, gene_4_rxn, protein_4_rxn)
            else:
                self.retrieve_rxn_info(rxn, rxn.get('id'), gene_4_rxn, protein_4_rxn)
        print ('{} number of new reactions added'.format(self.count_addedrxns))

    def fill_temp_array(self, krxn, cpdID, is_prod, stoic, temp_all_rxn_compound):
        '''
        Adds reaction information from metacyc xml file to temporary reaction
        list which later gets inserted into database
        '''
        if (krxn, cpdID, is_prod, stoic) not in temp_all_rxn_compound:
    ...
    def retrieve_rxn_info(self, rxn, rxnID, genes, proteins):
        '''
        ...
        temp_compartment = []
        for comp in rxn.findAll("speciesreference"):
            ...
            if compound_ID in self.store_compounds:
                temp_all_rxn_compound = self.fill_temp_array(rxnID,
                                                             self.store_compounds[compound_ID],
                                                             is_prod, stoic,
                                                             temp_all_rxn_compound)
            else:
                print ('WARNING: compound not in dictionary')
                temp_all_rxn_compound = self.fill_temp_array(rxnID, compound_ID, is_prod,
